 ...
 *                                      LOCAL VARIABLES
==================================================================================================*/
volatile uint8 DisplayReady = 0U;
/*==================================================================================================
 *                                      GLOBAL CONSTANTS
==================================================================================================*/
...
volatile unsigned char ScreenCharBuffer[CharacterRows][CharacterColumns] = {{"Hello World! =) "},
        {"                "},{"+----+----+----+"},{"SSD1306         "}};
...
I2c_RequestType OneByteCommandRequest ={0x3C, FALSE, FALSE, FALSE, FALSE, 2U, I2C_SEND_DATA, OneByteCommandBuffer};
I2c_RequestType TwoByteCommandRequest = {0x3C, FALSE, FALSE, FALSE, FALSE, 3U, I2C_SEND_DATA, TwoByteCommandBuffer};
I2c_RequestType ThreeByteCommandRequest = {0x3C, FALSE, FALSE, FALSE, FALSE, 4U, I2C_SEND_DATA, ThreeByteCommandBuffer};
I2c_RequestType DataWriteRequest = {0x3C, FALSE, FALSE, FALSE, FALSE, 2U, I2C_SEND_DATA, DataBuffer};
I2c_RequestType OptimizedDataWriteRequest = {0x3C, FALSE, FALSE, FALSE, FALSE, CharacterRows*CharacterColumns*8U+1U, I2C_SEND_DATA, AllDataBuffer};
...
void DisplayRefresh(void){
    if(DisplayReady == 1U){
        I2c_SyncTransmit(0U, &OptimizedDataWriteRequest);
        ...
        I2c_SyncTransmit(0U, &ThreeByteCommandRequest);
        /*set column range*/
...
void DisplayText(uint16 DisplayLine, const char Text[16], uint16 TextLength, uint16 TextOffset){
    ...
    for(uint8 i = TextOffset; i < 16U; i++){
        ScreenCharBuffer[DisplayLine][i]=Text[i - TextOffset];
    }
    ...
    for(uint16 DisplayCharacterIndex = StartIndex; DisplayCharacterIndex < StopIndex; DisplayCharacterIndex++){
        ...
        AsciiCharacter = ScreenCharBuffer[DisplayCharacterIndex / 16U][DisplayCharacterIndex % 16U];
...
void DisplayValue(uint16 DisplayLine, int Number, uint16 TextLength, uint16 TextOffset){
    ...
    for(uint8 i = 0; i < TextLength; i++){/*the string is written backwards again to restore its orientation*/
        if(DigitCount - i >= 0){
            ScreenCharBuffer[DisplayLine][i + TextOffset] = AsciiNumberString[DigitCount - i];
        }
        else{
            ScreenCharBuffer[DisplayLine][i + TextOffset] = ' ';
        }
    }
...
void DisplayInit(uint8 FontmapRotations){
    ...
    I2c_SyncTransmit(0U, &OneByteCommandRequest);
    /*display clock divide ratio*/
    TwoByteCommandBuffer[1] = 0xD5;
    TwoByteCommandBuffer[2] = 0x80;
    I2c_SyncTransmit(0U, &TwoByteCommandRequest);
    /*multiplex ratio*/
    ...
    I2c_SyncTransmit(0U, &TwoByteCommandRequest);
    DisplayReady = 1U;
    FontmapRotations = FontmapRotations % 4;
    for(uint8 i = 0; i < FontmapRotations; i++){
        RotateFontmap();
    }
